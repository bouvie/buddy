...
export const WithFooter: Story = {
  render: () => ({
    ...
    template: `
      <app-modal [isOpen]="true" title="Confirmer la suppression" size="sm">
        <p style="color:var(--k9-color-text-secondary);margin:0">
        ...
        <div modal-footer style="display:flex;justify-content:flex-end;gap:12px;padding:0 24px 24px">
          ...
          <app-button variant="danger">Supprimer</app-button>
        </div>
      </app-modal>
    `,
  }),
  ...
  render: () => ({
    ...
    template: `
      <div style="display:flex;flex-direction:column;gap:32px;padding:32px;background:var(--k9-color-bg)">
        <app-modal [isOpen]="true" title="Small — 400px" size="sm" [dismissible]="false">
          <p style="color:var(--k9-color-text-secondary);margin:0">Contenu SM.</p>
        </app-modal>
        <app-modal [isOpen]="true" title="Medium — 560px" size="md" [dismissible]="false">
          <p style="color:var(--k9-color-text-secondary);margin:0">Contenu MD.</p>
        </app-modal>
        <app-modal [isOpen]="true" title="Large — 760px" size="lg" [dismissible]="false">
